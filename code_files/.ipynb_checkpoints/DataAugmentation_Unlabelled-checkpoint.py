...
class DataAugmentation:
    ...
    def random_swap(self): # n is number of times to swap randomly 2 words
        ...
        targets_sentences_temp=[]
        i=0
    
        for sentences_src, sentences_tar in zip(source_sentences,target_sentences):
            
            sentences_src = sentences_src.split()
            sentences_tar = sentences_tar.split()
            
            for _ in range(self.num_sentences):
                
                sentences_src_tmp = sentences_src.copy()
                sentences_tar_tmp = sentences_tar.copy()
                
                for _ in range(self.swap_words):
                    sentences_src_tmp = self.swap_word(sentences_src_tmp)
                    sentences_tar_tmp = self.swap_word(sentences_tar_tmp)
                
                sentences_str_src = ' '.join(sentences_src_tmp)
                sentences_str_tar = ''.join(sentences_tar_tmp)
                source_sentences_temp.append(sentences_str_src)
                targets_sentences_temp.append(sentences_str_tar)
    #         break
            i+=1

        source_sentences.extend(source_sentences_temp)
        target_sentences.extend(targets_sentences_temp)
    ...
    def random_deletion(self):
        ...
        for sentences_src, sentences_tar in zip(source_sentences,target_sentences):
            ...
            if len(sentences_src) == 1 or len(sentences_tar) == 1:
                    pass
            ...
            for _ in range(self.num_sentences):
                ...
                source_sentences_temp=[]
                target_sentences_temp=[]
                
                for word_src,word_tar in zip(sentences_src,sentences_tar):
                    r = random.uniform(0, 1)
                    if r > self.del_words_prob:
                        source_sentences_temp.append(word_src)
                        target_sentences_temp.append(word_tar)
                ...
                if len(source_sentences_temp) == 0: #if you end up deleting all words, just return a random word
                    ...
                    source_sentences_temp.append(sentences_src[rand_int])
                if len(source_sentences_temp) == 0: #if you end up deleting all words, just return a random word
                    rand_int = random.randint(0, len(target_sentences_temp)-1)
                    target_sentences_temp.append(sentences_tar[rand_int])
                
                
                sentences_str_src = ' '.join(source_sentences_temp)
                sentences_str_tar = ' '.join(target_sentences_temp)
                senetences_temp_src.append(sentences_str_src)
                senetences_temp_tar.append(sentences_str_tar)

    #         break
            i+=1
        source_sentences.extend(senetences_temp_src)
        target_sentences.extend(senetences_temp_tar)    
        column_names = ["source","target"]
        df = pd.DataFrame(columns=column_names,dtype=object)
        df = df.assign(source=source_sentences)
        df = df.assign(target = target_sentences)
...
#     new_words = words.copy()
        source_sentences  = list(self.dataframe.source)
        target_sentences = list(self.dataframe.target)
        senetences_temp_src_lst=[]
        senetences_temp_tar_lst= []
        k=0
        for sentences_src,sentences_tar in zip(source_sentences,target_sentences):
    #         dict={}
            ...
            for _ in range(self.num_sentences):
                ...
                sentences_tmp_src = sentences_src.copy()
                sentences_tmp_tar = sentences_tar.copy()
